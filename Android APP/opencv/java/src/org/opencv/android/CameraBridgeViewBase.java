...
//    }
    ...
    private void updateMatrix() {
        ...
        mMatrix.reset();
        if (isFrontCamera) {
            mMatrix.preScale(-1, 1, hw, hh); // This will mirror the front camera
        }
        mMatrix.preTranslate(hw, hh);

        // Ensure that the camera output is rotated for portrait mode (no landscape)
        // For front camera, rotate 0 degrees, and for back camera, rotate 0 degrees.
        if (isFrontCamera) {
            mMatrix.preRotate(0); // No rotation for front camera
        } else {
            mMatrix.preRotate(0); // No rotation for back camera
        }
