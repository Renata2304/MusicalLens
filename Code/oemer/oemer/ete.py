...
def resize_if_needed(image_path: str) -> str:
    ...
    if img.width > MAX_WIDTH:
        ...
        tmp_path = image_path.replace(".", "_resized.")
        img.save(tmp_path)
        return tmp_path
    return image_path
...
def polish_symbols(rgb_black_th=300):
    ...
    arr = cv2.morphologyEx(arr.astype(np.uint8), cv2.MORPH_CLOSE, ker)
...
def extract(args: Namespace) -> str:
    ...
    img_path = resize_if_needed(img_path)
